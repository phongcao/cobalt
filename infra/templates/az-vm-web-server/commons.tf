...
locals {
  ...
  base_name_12 = length(local.base_name) < 13 ? local.base_name : "${substr(local.base_name, 0, 12 - length(local.suffix))}${local.suffix}"
  base_name_83 = length(local.base_name) < 84 ? local.base_name : "${substr(local.base_name, 0, 83 - length(local.suffix))}${local.suffix}"

  // Resolved resource names
  app_rg_name         = "${local.base_name_83}-app-rg" // app resource group (max 90 chars)
  vm_name             = "${local.base_name_12}-vm"     // virtual machine
}
